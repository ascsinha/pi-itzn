...
class Usuario(UserMixin, db.Model):
    ...
    nome: so.Mapped[str] = so.mapped_column(sa.String(64), nullable = False, index = True)
    email: so.Mapped[str] = so.mapped_column(sa.String(128), nullable = False, unique = True, index = True)
    cpf: so.Mapped[Optional[str]] = so.mapped_column(sa.String(14), unique = True, nullable = True)
    data_nascimento: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, nullable = True)
    telefone: so.Mapped[Optional[str]] = so.mapped_column(sa.String(13), unique = True, nullable = True, index = True)
